...
nu = float(sys.argv[3])
sm = lsm.load(sys.argv[1])
pm = sm.getColValues('Patch') == sys.argv[2]
s0 = sm.getColValues('I')[pm]
si = sm.getColValues('SpectralIndex')[pm]
nu_ref = np.mean(sm.getColValues('ReferenceFrequency')[pm])
print(f'Flux={np.sum(s0)} at reference frequency={nu_ref}MHz')

logflux = 0
for i in range(len(si[0])):
    logflux += si[:,i]*np.log10(nu/nu_ref)**(i+1)
    # print(np.sum(10**logflux))

# print(10**logflux, np.sum(10**logflux))
print(f'{np.sum(s0*10**logflux):.2f}Jy at {nu:.2f}MHz')
